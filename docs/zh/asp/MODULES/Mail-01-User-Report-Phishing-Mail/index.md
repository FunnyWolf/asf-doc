...
ASP 提供的样例模块都包含用于测试的告警数据,位于 `DATA/{模块名}/mock_alert.py`,执行该脚本即可将测试告警数据导入 Redis Stream 中.
...
- SIEM 平台中规则 `Mail-01-User-Report-Phishing-Mail` 会将所有用户上报的钓鱼邮件元数据作为告警发送到Redis Stream中
- ASP 模块 `Mail-01-User-Report-Phishing-Mail` 使用 Langgraph 构建的 AI Agent 分析每条告警,确认是否为钓鱼邮件,并格式化为 SIRP Alert 的格式.
