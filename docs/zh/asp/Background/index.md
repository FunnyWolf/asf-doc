...
| 后台任务                               | 职责                                    |
|------------------------------------|---------------------------------------|
| [模块运行框架](module-engine/index.md)   | 持续消费 Redis Stream，将原始告警处理为 Alert/Case |
| [Playbook 执行](playbook-execution/index.md) | 轮询待执行的 Playbook 记录，分派到线程池运行           |
| [自动化分析](auto-analysis/index.md)            | 调度到期 Case 并调用 LLM 生成调查报告              |
