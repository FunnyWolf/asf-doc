...
Module worker 会扫描官方目录和 `custom\modules\*.py`，发现其中继承 `BaseModule` 的 `Module` 类，然后按 `STREAM_NAME` 消费 Redis Stream。同名文件由 `custom` 目录覆盖官方示例。
