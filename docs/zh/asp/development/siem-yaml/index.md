...
Compose 部署时，用户自定义 SIEM YAML 放在：
...
```text
backend\data\siem\*.yaml
```

每个 YAML 文件描述一个可查询索引。同名 YAML 由 `custom` 目录覆盖官方配置。

当前后端会从该目录加载配置。只有写入 YAML 的索引才会出现在 `siem_explore_schema` 中。
