...
| Harness Agent 集成 | 通过插件和 MCP 连接 ClaudeCode / Codex / OpenCode 等 Harness Agent，让 Agent 可以操作 Case、搜索日志、查询威胁情报、编写模块和剧本。 |
| 知识持续积累           | 从已关闭 Case 中提取可复用经验，构建组织级安全知识库。                                                                    |
| 协作与治理 | Comments、Inbox、Audit Log、用户/API Key 和 LDAP 提供协作、通知、审计和访问控制能力。 |
