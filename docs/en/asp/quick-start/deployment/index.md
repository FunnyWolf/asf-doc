...
After modifying scripts or YAML, execute `Refresh / Validate` in the corresponding [Custom Console](../../development/custom-console/) tab. If Python dependencies or common helper modules are changed, re-execute `asp-custom-deps` and restart related containers.
